# AuthResult.gd
# Shown after successful authorize() -- displays address + cache status.
# User taps "Continue" to proceed to Dashboard.
...
func _ready() -> void:
# Pull data passed via SceneManager or from plugin directly
var address := ""
...
var t := create_tween()
t.tween_property(continue_btn, "scale", Vector2(1.0, 1.0), DesignTokens.ANIM_FAST) \
.set_trans(Tween.TRANS_BACK).set_ease(Tween.EASE_OUT)
