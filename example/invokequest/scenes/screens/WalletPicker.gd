# WalletPicker.gd
# Wallet selection screen -- shows Phantom, Backpack, Solflare.
# Triggers authorize() on card tap. Animates cards in with stagger.
# Reference: SOLANAQUEST_ASSET_MAP.md Section 6C.
...
# Hook up plugin signals if available
...
t.tween_property(card, "modulate:a", 1.0, DesignTokens.ANIM_NORMAL) \
.set_delay(i * DesignTokens.ANIM_STAGGER_WALLET)
t.tween_property(card, "position:y", rest_y, DesignTokens.ANIM_SLOW) \
.set_delay(i * DesignTokens.ANIM_STAGGER_WALLET) \
...
if _mwa == null:
# Desktop fallback -- simulate success for testing
