# SignMessage.gd
# Demonstrates sign_messages() -- off-chain personal signing.
# User types a message, wallet signs it, signed bytes are displayed.
...
_show_result("SimulatedSignedBytes_" + msg.substr(0, 8).hex_encode())
return

# Encode message as UTF-8 bytes
var msg_bytes: PackedByteArray = msg.to_utf8_buffer()
# Address bytes -- use cached address or empty
