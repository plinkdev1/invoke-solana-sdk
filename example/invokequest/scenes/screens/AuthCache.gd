# AuthCache.gd
# Live demo of the auth token cache -- the key grant feature.
# Shows token status, age, address. Lets user clear + reconnect.
# Reference: SOLANAQUEST_ASSET_MAP.md Section 6G.
...
@onready var clear_btn:       Button    = $Scroll/VBox/ClearBtn
...
_add_log("Reauthorize success -- " + address.substr(0, 8) + "...")
reconnect_btn.disabled = false
_refresh_status()
