# Dashboard.gd
# Main hub screen -- address, balance, quick actions, disconnect.
# Reference: SOLANAQUEST_ASSET_MAP.md Section 6D.
...
@onready var disconnect_btn: Button = $Scroll/VBox/DisconnectBtn
...
# Simple count-up tween on the balance label text
var tween := create_tween()
tween.tween_method(_set_balance_text, 0.0, target, 0.8) \
...
t.tween_property(card, "scale", Vector2(1.0, 1.0), DesignTokens.ANIM_SLOW) \
...
bt.tween_property(btn, "modulate:a", 1.0, DesignTokens.ANIM_NORMAL) \
.set_delay(DesignTokens.ANIM_SLOW + i * DesignTokens.ANIM_STAGGER_ACTION)
bt.tween_property(btn, "position:y", btn.position.y - 16.0, DesignTokens.ANIM_NORMAL) \
.set_delay(DesignTokens.ANIM_SLOW + i * DesignTokens.ANIM_STAGGER_ACTION)
